...
class IAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        context = self.embed(text_raw_indices) # (128, 80, 300)
        aspect = self.embed(aspect_indices) # (128, 80, 300)
        context, (_, _) = self.lstm_context(context, text_raw_len) # (128, 36, 300)
        aspect, (_, _) = self.lstm_aspect(aspect, aspect_len) # (128, 3, 300)

        aspect_len = torch.tensor(aspect_len, dtype=torch.float).to(self.opt.device)
        aspect_pool = torch.sum(aspect, dim=1)
        aspect_pool = torch.div(aspect_pool, aspect_len.view(aspect_len.size(0), 1)) # (128, 300)

        text_raw_len = torch.tensor(text_raw_len, dtype=torch.float).to(self.opt.device)
        context_pool = torch.sum(context, dim=1)
        context_pool = torch.div(context_pool, text_raw_len.view(text_raw_len.size(0), 1)) # (128, 300)
